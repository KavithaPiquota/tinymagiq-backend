...
const getUserIdByIdentifier = async (identifier, first_name, last_name) => {
    ...
    if (identifier) {
        ...
        throw new Error('User identifier (email, username, or first_name and last_name) is required');
    }
    if (result.rows.length === 0) {
...
const getMentorPods = async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            data: result.rows,
            message: result.rows.length > 0 ? 'Mentor pods fetched successfully' : 'No pods assigned to mentor'
        });
    } catch (error) {
...
const getMentorPodConcepts = async (req, res) => {
    ...
    try {
        ...
        const pods = await Promise.all(podsResult.rows.map(async (pod) => {
        ...
        res.json({
            success: true,
            data: pods,
            message: pods.length > 0 ? 'Mentor pod concepts fetched successfully' : 'No pods or concepts assigned to mentor'
        });
    } catch (error) {
        if (['Mentor not found'].includes(error.message)) {
            return res.status(400).json({
...
const getMentorOrguserProgress = async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            data: pods,
            message: pods.length > 0 ? 'Mentor orguser progress fetched successfully' : 'No pods or users assigned to mentor'
        });
    } catch (error) {
        if (['Mentor not found'].includes(error.message)) {
            return res.status(400).json({
...
const getMentorOrguserDetails = async (req, res) => {
    ...
    try {
        ...
        if (['Mentor not found', 'User not found or not an orguser', 'User identifier (email, username, or first_name and last_name) is required'].includes(error.message)) {
            return res.status(400).json({
                success: false,
                error: 'Bad request',
                message: error.message
        ...
        if (error.message === 'Orguser is not assigned to any of the mentor\'s pods') {
            return res.status(403).json({
                success: false,
                error: 'Forbidden',
                message: error.message
