...
const addOrganization = async (req, res) => {
    ...
    if (typeof max_users_per_batch !== 'number' || max_users_per_batch <= 0) {
        return res.status(400).json({ error: 'max_users_per_batch must be a positive number' });
    }
    if (typeof max_users_per_pod !== 'number' || max_users_per_pod <= 0) {
        return res.status(400).json({ error: 'max_users_per_pod must be a positive number' });
    }
    ...
    try {
        ...
        const result = await pool.query(
            ...
            [name, description || null, is_active, max_users_per_batch, max_users_per_pod]
        );
