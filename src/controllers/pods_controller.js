...
const getBatchIdByName = async (batch_name, organization_id) => {
    const result = await pool.query('SELECT batch_id, batch_size, pod_size, is_active FROM batches WHERE batch_name = $1 AND organization_id = $2', [batch_name, organization_id]);
    if (result.rows.length === 0) {
...
const validatePodSize = async (batch_id, pod_size, exclude_pod_id = null) => {
    const batchResult = await pool.query('SELECT batch_size, pod_size FROM batches WHERE batch_id = $1', [batch_id]);
    const { batch_size, pod_size: batch_pod_size } = batchResult.rows[0];
    
    const podsResult = await pool.query(
        'SELECT COUNT(*) as pod_count FROM pods WHERE batch_id = $1 AND is_active = TRUE' + (exclude_pod_id ? ' AND pod_id != $2' : ''),
        exclude_pod_id ? [batch_id, exclude_pod_id] : [batch_id]
    );
    const pod_count = parseInt(podsResult.rows[0].pod_count);
    
    if ((pod_count + 1) * batch_pod_size > batch_size) {
        throw new Error(`Adding this pod would exceed the batch size of ${batch_size}. Maximum ${Math.floor(batch_size / batch_pod_size)} pods allowed.`);
    }
};

const addPod = async (req, res) => {
    const { organization_name, batch_name, mentor_email, mentor_id, pod_name, is_active = true } = req.body;

    if (!organization_name || !batch_name || !pod_name || (!mentor_email && !mentor_id)) {
        return res.status(400).json({
            success: false,
            error: 'Bad request',
            message: 'Organization name, batch name, pod name, and mentor email or ID are required'
        });
    }
    ...
    try {
        ...
        await validatePodSize(batch.batch_id, batch.pod_size);
        ...
        if (['Organization not found', 'Batch not found', 'Mentor not found or not a valid mentor', 'Cannot create or update pod for an inactive batch'].includes(error.message)) {
            return res.status(400).json({
                success: false,
                error: 'Bad request',
        ...
        if (error.message.includes('Adding this pod would exceed the batch size')) {
            return res.status(400).json({
                success: false,
                error: 'Bad request',
                message: error.message
...
const updatePod = async (req, res) => {
    ...
    const { organization_name, batch_name, mentor_email, mentor_id, pod_name, is_active } = req.body;

    if (!organization_name && !batch_name && !mentor_email && !mentor_id && !pod_name && is_active === undefined) {
        return res.status(400).json({
            success: false,
            error: 'Bad request',
    ...
    try {
        ...
        if (organization_name || batch_name) {
            organization_id = organization_name ? await getOrganizationIdByName(organization_name) : null;
            batch = batch_name ? await getBatchIdByName(batch_name, organization_id || (await pool.query('SELECT organization_id FROM pods WHERE pod_id = $1', [pod_id])).rows[0].organization_id) : null;
        }
        if (mentor_email || mentor_id) {
            mentor = await getMentorIdByEmailOrId(mentor_email, mentor_id);
        }
        if (batch && is_active) {
            await validatePodSize(batch.batch_id, batch.pod_size, pod_id);
        }
        ...
        if (organization_name) { fields.push(`organization_id = $${index++}`); values.push(organization_id); }
        if (batch_name) { fields.push(`batch_id = $${index++}`); values.push(batch.batch_id); }
        if (mentor_email || mentor_id) { fields.push(`mentor_id = $${index++}`); values.push(mentor.user_id); }
        if (pod_name) { fields.push(`pod_name = $${index++}`); values.push(pod_name); }
        ...
        if (fields.length > 0) {
            ...
            const result = await pool.query('SELECT * FROM pods WHERE pod_id = $1 AND is_active = TRUE', [pod_id]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    error: 'Not found',
...
const getAllPods = async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT p.*, b.batch_name, b.batch_size, b.pod_size, b.is_active AS batch_is_active, o.organization_name, u.user_id, u.first_name, u.last_name, u.email ' +
            'FROM pods p ' +
