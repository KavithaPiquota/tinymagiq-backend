        ...
        try {
            ...
            try {
                ...
                if (finalStatus === 'completed' && Object.values(scoringFields).some(val => val !== null)) {
                    ...
                    updateParams = [JSON.stringify(conversation), finalStatus, finalStage, finalConceptName, parseInt(chat_id)];
                }
                ...
                if (finalStatus === 'completed' && (updatedChat.final_weighted_score !== null || updatedChat.six_facets_average !== null)) {
                    responseData.scoring = {
                        six_facets: {
    ...
    async getChatHistory(req, res, next) {
        try {
            ...
            const chats = result.rows.map(chat => {
                ...
                if (chat.status === 'completed' && (chat.final_weighted_score !== null || chat.six_facets_average !== null)) {
                    chatData.scoring = {
                        six_facets: {
